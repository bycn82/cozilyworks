/*
 * $Id: RequestUtils.java 76098 2004-11-17 07:07:32Z mrdon $ Copyright 1999-2004 The Apache Software
 * Foundation. Licensed under the Apache License, Version 2.0 (the "License"); you may not use this
...
/**
 * <p>
 * General purpose utility methods related to processing a servlet request in the Struts controller
 * framework.
 * </p>
 * 
...
public class RequestUtils{
	...
	/**
	 * <p>
	 * Create and return an absolute URL for the specified context-relative path, based on the
	 * server and context information in the specified request.
	 * </p>
	 * 
	 * @param request
	...
	/**
	 * <p>
	 * Return the <code>Class</code> object for the specified fully qualified class name, from
	 * this web application's class loader.
	 * </p>
	 * 
	 * @param className
	...
	/**
	 * <p>
	 * Return a new instance of the specified fully qualified class name, after loading the class
	 * from this web application's class loader. The specified class <strong>MUST</strong> have a
	...
	/**
	 * <p>
	 * Create (if necessary) and return an <code>ActionForm</code> instance appropriate for this
	 * request. If no <code>ActionForm</code> instance is required, return <code>null</code>.
	 * </p>
	 * 
	 * @param request
	 *            The servlet request we are processing
	...
	/**
	 * <p>
	 * Determine whether <code>instance</code> of <code>ActionForm</code> is suitable for re-use
	 * as an instance of the form described by <code>config</code>.
	 * </p>
	 * 
	 * @param instance
	 *            an instance of <code>ActionForm</code> which was found, probably in either
	 *            request or session scope.
	 * @param config
	 *            the configuration for the ActionForm which is needed.
	 * @return true if the instance found is "compatible" with the type required in the
	 *         <code>FormBeanConfig</code>; false if not, or if <code>instance</code> is null.
	 * @throws ClassNotFoundException
	 *             if the <code>type</code> property of <code>config</code> is not a valid Class
	 *             name.
	 */
	private static boolean canReuseActionForm(ActionForm instance,FormBeanConfig config) throws ClassNotFoundException{
		...
		if(log.isDebugEnabled()){
			log.debug(" Can recycle existing "+formType+" instance "+"of type '"+className+"'?: "+canReuse);
			log.trace(" --> "+instance);
		}
		return(canReuse);
	}
	/**
	 * <p>
	 * Create and return an <code>ActionForm</code> instance appropriate to the information in
	 * <code>config</code>.
	 * </p>
	 * <p>
	...
	public static ActionForm createActionForm(FormBeanConfig config,ActionServlet servlet){
		...
		try{
			...
			if(log.isDebugEnabled()){
				log.debug(" Creating new "+(config.getDynamic()?"DynaActionForm":"ActionForm")+" instance of type '"
					+config.getType()+"'");
				log.trace(" --> "+instance);
			}
		}catch(Throwable t){
	 ...
	 * @param locale
	 *            Name of the session attribute for our user's Locale. If this is <code>null</code>,
	...
	/**
	 * <p>
	 * Populate the properties of the specified JavaBean from the specified HTTP request, based on
	 * matching each parameter name against the corresponding JavaBeans "property setter" methods in
	 * the bean's class. Suitable conversion is done for argument types as described under
	 * <code>convert()</code>.
	 * </p>
	 * 
	 * @param bean
	 ...
	 * <p>
	 * Populate the properties of the specified JavaBean from the specified HTTP request, based on
	 * matching each parameter name (plus an optional prefix and/or suffix) against the
	 * corresponding JavaBeans "property setter" methods in the bean's class. Suitable conversion is
	 * done for argument types as described under <code>setProperties</code>.
	 * </p>
	 * <p>
	 * If you specify a non-null <code>prefix</code> and a non-null <code>suffix</code>, the
	 * parameter name must match <strong>both</strong> conditions for its value(s) to be used in
	 * populating bean properties. If the request's content type is "multipart/form-data" and the
	 * method is "POST", the <code>HttpServletRequest</code> object will be wrapped in a
	...
	/**
	 * <p>
	 * Try to locate a multipart request handler for this request. First, look for a
	 * mapping-specific handler stored for us under an attribute. If one is not present, use the
	 * global multipart handler, if there is one.
	 * </p>
	 * 
	 * @param request
	 ...
	 * @exception ServletException
	 *                if any exception is thrown while attempting to locate the multipart handler.
	 */
	...
	/**
	 * <p>
	 * Create a <code>Map</code> containing all of the parameters supplied for a multipart
	 * request, keyed by parameter name. In addition to text and file elements from the multipart
	 * body, query string parameters are included as well.
	 * </p>
	 * 
	 * @param request
	 *            The (wrapped) HTTP request whose parameters are to be added to the map.
	...
	/**
	 * <p>
	 * Compute the printable representation of a URL, leaving off the scheme/host/port part if no
	 * host is specified. This will typically be the case for URLs that were originally created from
	 * relative or context-relative URIs.
	 * </p>
	 * 
	 * @param url
	...
	/**
	 * <p>
	 * Return the context-relative URL that corresponds to the specified {@link ActionConfig},
	 * relative to the module associated with the current modules's {@link ModuleConfig}.
	 * </p>
	 * 
	 * @param request
	 *            The servlet request we are processing
	...
	/**
	 * <p>
	 * Return the context-relative URL that corresponds to the specified <code>ForwardConfig</code>.
	 * The URL is calculated based on the properties of the {@link ForwardConfig} instance as
	 * follows:
	 * </p>
	 * <ul>
	 * <li>If the <code>contextRelative</code> property is set, it is assumed that the
	 * <code>path</code> property contains a path that is already context-relative:
	 * <ul>
	 * <li>If the <code>path</code> property value starts with a slash, it is returned
	 * unmodified.</li>
	 * <li>If the <code>path</code> property value does not start with a slash, a slash is
	 * prepended.</li>
	 * </ul>
	 * </li>
	 * <li>Acquire the <code>forwardPattern</code> property from the
	 * <code>ControllerConfig</code> for the application module used to process this request. If
	 * no pattern was configured, default to a pattern of <code>$M$P</code>, which is compatible
	 * with the hard-coded mapping behavior in Struts 1.0.</li>
	 * <li>Process the acquired <code>forwardPattern</code>, performing the following
	 * substitutions:
	 * <ul>
	 * <li><strong>$M</strong> - Replaced by the module prefix for the application module
	 * processing this request.</li>
	 * <li><strong>$P</strong> - Replaced by the <code>path</code> property of the specified
	 * {@link ForwardConfig}, prepended with a slash if it does not start with one.</li>
	 * <li><strong>$$</strong> - Replaced by a single dollar sign character.</li>
	 * <li><strong>$x</strong> (where "x" is any charater not listed above) - Silently omit these
	 * two characters from the result value. (This has the side effect of causing all other $+letter
	 * combinations to be reserved.)</li>
	 * </ul>
	 * </li>
	 * </ul>
	 * 
	 ...
	 * Server-relative URLs can be created by simply appending the server-relative path (starting
	 * with '/') to this.
	 * </p>
	 * 
	 * @param request
	...
	public static StringBuffer requestToServerUriStringBuffer(HttpServletRequest request){
		StringBuffer serverUri=
			createServerUriStringBuffer(request.getScheme(),request.getServerName(),request.getServerPort(),request
				.getRequestURI());
		return serverUri;
	}
	/**
	 * <p>
	 * Return <code>StringBuffer</code> representing the scheme, server, and port number of the
	 * current request. Server-relative URLs can be created by simply appending the server-relative
	 * path (starting with '/') to this.
	 * </p>
	 * 
	 * @param request
	 *            The servlet request we are processing
	...
	/**
	 * <p>
	 * Select the module to which the specified request belongs, and add corresponding request
	 * attributes to this request.
	 * </p>
	 * 
	 * @param prefix
	 ...
	 * @since Struts 1.1
	 * @deprecated Use
	 *             {@link org.apache.struts.util.ModuleUtils#selectModule(String,HttpServletRequest,ServletContext)}
	 ...
	 *            The ServletContext for this web application
	 * @deprecated Use
	 *             {@link org.apache.struts.util.ModuleUtils#selectModule(HttpServletRequest,ServletContext)}
	 *             instead. This will be removed after Struts 1.2.
	 */
	 ...
	 * @deprecated Use Use
	 *             {@link org.apache.struts.util.ModuleUtils#getModuleName(HttpServletRequest,ServletContext)}
	 *             instead. This will be removed after Struts 1.2.
	 */
	 ...
	 * @return The module prefix or ""
	 * @deprecated Use
	 *             {@link org.apache.struts.util.ModuleUtils#getModuleName(String,ServletContext)}
	 *             instead. This will be removed after Struts 1.2.
	 */
	...
	/**
	 * <p>
	 * Return the current <code>ModuleConfig</code> object stored in request, if it exists, null
	 * otherwise. This method can be used by a {@link org.apache.struts.action.PlugIn} to retrieve
	 * the current module config object. If no moduleConfig is found, this means that the request
	 * hasn't hit the server through the Struts servlet. The appropriate module config can be set
	 * and found with
	 * <code>{@link RequestUtils#selectModule(HttpServletRequest, ServletContext)} </code>.
	 * 
	 * @param request
	 *            The servlet request we are processing
	 * @return the ModuleConfig object from request, or null if none is set in the request.
	 * @since Struts 1.1
	 * @deprecated Use
	 *             {@link org.apache.struts.util.ModuleUtils#getModuleConfig(HttpServletRequest)}
	 *             instead. This will be removed after Struts 1.2.
	 */
	 ...
	 * @since Struts 1.1
	 * @deprecated Use
	 *             {@link org.apache.struts.util.ModuleUtils#getModuleConfig(HttpServletRequest,ServletContext)}
	 *             instead. This will be removed after Struts 1.2.
	 */
	...
	/**
	 * <p>
	 * Return the list of module prefixes that are defined for this web application. <strong>NOTE</strong> -
	 ...
	 * @since Struts 1.1
	 * @deprecated Use {@link org.apache.struts.util.ModuleUtils#getModulePrefixes(ServletContext)}
	 *             instead. This will be removed after Struts 1.2.
	 */
