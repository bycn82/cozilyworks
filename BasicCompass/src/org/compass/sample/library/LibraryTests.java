/*
 * Copyright 2004-2009 the original author or authors. Licensed under the Apache License, Version 2.0 (the "License");
...
/**
 * Some tests that show the power of compass using a simple library domain model. Populated with some books and authors,
 * and by no means it is a complete list.
 * 
 * @author kimchy
 */
public class LibraryTests extends TestCase{
	...
	protected void setUp() throws Exception{
		super.setUp();
		CompassConfiguration config=new CompassConfiguration().configure("/org/compass/sample/library/compass.cfg.xml").addClass(Author.class)
			.addClass(Article.class).addClass(Book.class);
	...
	public void setUpData() throws Exception{
		...
		shogun.setSummary("A story of a hero who is not a person but a place and a time,"+" medieval Japan on the threshold of becoming a sea power");
	...
	public static void main(final String[] args) throws Exception{
		...
		libraryTests.compassTemplate.execute(new CompassCallbackWithoutResult(){
			protected void doInCompassWithoutResult(CompassSession session) throws CompassException{
				...
				System.out.println("Found ["+hits.getLength()+"] hits for [name:jack] query");
				System.out.println("======================================================");
	...
	public static void print(CompassHits hits,int hitNumber){
		...
		System.out.println("ALIAS ["+resource.getAlias()+"] ID ["+((Identifiable)value).getId()+"] SCORE ["+hits.score(hitNumber)+"]");
